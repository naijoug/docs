...
以下 `skill.yaml` 字段是本文示例 linker/installer 的约定，不是所有 agent 平台通用的技能规范。实际仓库未使用该体系时，不要为了套用此清单新增旁路元数据；先以目标运行环境的技能格式为准。
...
元数据齐全只能证明入口有内容。还应对照 [OpenAI 的 Skills 实践](https://developers.openai.com/blog/rethinking-skills-and-prompts-for-gpt-6-astra#better-skills) 检查选择是否准确：description 简短说明具体适用任务；多流程技能的根文档只做路由，细节按需读取。

| 检查 | 最小证据 |
| --- | --- |
| 正向触发 | 一个确实需要该工作流的任务能选中技能 |
| 相邻反例 | 只共享关键词、但不需要该工作流的任务不会误触发 |
| 按需加载 | 单一路径任务不会被要求读完其他流程的 references |
| 决策边界 | 已授权步骤能继续；真正缺少授权时指出具体动作及规则 |
| 模型兼容 | 记录实际使用的模型、工具和重复运行结果，不假设每个模型表现相同 |

例如“新增或审查数据库 migration”比“涉及数据库、查询或持久化就使用”更准确。关键词只辅助发现，不应代替任务意图判断。发现重复安装来源或全局常驻技能时，先查明有效来源和用户偏好；不要在修正文档时顺便修改个人环境。
