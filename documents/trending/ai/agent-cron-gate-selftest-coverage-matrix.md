...
| 首条时间标题前存在正文 | `summaries/scripts/test-check-hermes-notebook.sh` | 文件开头先写前言或说明，再出现第一条 `## HH:mm` | checker 报 content before first time heading，确保 daily notebook 直接由时间条目开始 |
