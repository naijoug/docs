...
先提供目标、必要上下文、边界和验收标准。对于范围明确的任务，模型可以自行选择阅读顺序和实现步骤；只有顺序会影响正确性时，才额外规定流程。
...
角色只在专业视角影响结果时补充；输出 schema 只在下游需要解析或固定交付物时规定；流程只在迁移顺序、事务或人工决策等确有依赖的场景规定。不要为了凑模板，要求每次复述需求或先提交一份方案。

对 GPT-6 Astra 的调整依据见 [官方模型指南](https://developers.openai.com/api/docs/guides/latest-model?model=gpt-6-astra) 与 [Skills 和提示词实践](https://developers.openai.com/blog/rethinking-skills-and-prompts-for-gpt-6-astra#better-skills)：保持触发范围准确、按需加载材料，并明确完成标准。共享模板还应通过实际使用的其他模型验证，不能把单一模型的行为当成普遍保证。
...
验收：
- 完成实现并运行适用的验证命令：{测试/构建/类型检查命令}。
- 修复本次改动造成的失败，直到结果满足验收标准。
- 报告结果、实际验证及有影响的未验证项。

自主执行：
- 按任务需要读取文件，常规可逆选择可说明假设后继续。
- 仅当缺失信息会实质改变结果且无法从上下文解决时提问；继续不依赖答案的工作。

限制：
- 不要大规模重构。
- 不要引入新依赖，除非说明必要性。
- 不用 TODO 代替已授权且可完成的工作；确实受阻时说明具体缺口。
```
...
# Decision Boundaries
保留本次会话中已有的授权，仅在以下动作尚未被授权时等待确认：
- 对生产数据库执行结构或数据变更
- 部署到指定生产环境、发送对外邮件
- 调用付费 API 超过 {额度}

本地 migration 草稿、测试和预览按 Allowed Actions 执行。
需要确认外部动作时，先完成已授权的准备工作，给出可审查结果。
若某条技能规则造成暂停，引用规则来源并指出被阻塞的动作。
...
3. 固定模型版本、输入、工具权限和模型支持的推理/输出设置；记录实际参数。
4. 每个样本重复运行，比较正确性、完成率、必要验证遗漏、格式、引用、重复确认、工具调用数与耗时。
5. 保留证据支持的版本，记录样本和适用场景；静态检查不能证明 agent 表现提升。

参数不能跨模型照搬。例如 GPT-6 Astra 不支持 `temperature`、`top_p` 和 `top_logprobs`，应按 [官方迁移参数说明](https://developers.openai.com/api/docs/guides/latest-model?model=gpt-6-astra) 配置。比较提示词时保持其余可用设置一致。
...
工作方式：
- 在已授权范围内持续完成任务及适用验证。
- 只说明影响结果的假设；必要提问不阻塞独立工作。
- 达到质量标准后交付，不默认停在首版等待确认。
...
| 缺少完成边界 | 可能停在首版或扩大范围 | 明确验收，允许连续完成内部小步骤 |
