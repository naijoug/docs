...
2. Plan：列出 2-4 个候选任务，并写清为什么选其中一个。
...
| 候选工作有哪些 | 列出 2-4 个具体文件或成果形状 | “写作、赚钱、学习 AI” |
...
如果 `git diff --cached --name-only` 出现本轮没有记录的文件，先 `git restore --staged <path>`。不要因为仓库里已有 dirty path，就把它们顺手提交成“清理”。
