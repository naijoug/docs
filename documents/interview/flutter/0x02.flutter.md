...
> `Flutter` 与原生（`Android` 和 `iOS`）的通信实际上是通过 `Flutter` 的平台通道（`Platform Channels`）来实现的。`Flutter` 框架和宿主操作系统（`Android` 或者 `iOS`）之间的通信是建立在消息传递的机制上的。在 `Flutter` 端和宿主端可以发送和接收消息。

  - **在 Flutter 端**，可以使用 `MethodChannel` 或者 `EventChannel` 实例来发送和接收消息。它们都有一个唯一的名称，在宿主端和 `Flutter` 端建立连接，提供消息通道。

  - **在宿主端**，Android 使用 `MethodChannel` 或者 `EventChannel` 类，iOS 使用 `FlutterMethodChannel` 或者 `FlutterEventChannel` 类，这两个类也都有一个与 Flutter 端相对应的名称。

  - 数据交换采用的是**序列化的 JSON 文本**，所以发送的参数需要是能够转换成 `JSON` 的类型。

注意:虽然 `Flutter` 提供了这样的桥接机制，但 `Flutter` 的设计理念是尽可能地减少 `Flutter` 和原生之间的交互，这主要是为了保持应用程序的高性能和跨平台的一致性。所以建议只在必要的情况下使用这种机制，例如访问平台特有的 `API` 或者调用原生的一些库。

:::

### ❓`Platform Channels` 的底层原理

::: details 💡

> `Platform Channels`：平台通道，是 `Flutter` 和原生代码进行通讯的主要方式，主要依赖于两个库：`Flutter` 库和 `Flutter engine` 库。

  - 在 `Flutter` 库中，有一个`Platform Channels`类，它用于定于通道名称、通道类型、解编码器等。

  - 在 `Flutter engine` 库中，也有一个 `Platform Channels` 类，类似于 `Flutter` 库中的类，用于接收消息和向通道中发送消息。

平台通道实际上是基于消息传递进行通讯，具体流程：

  - 当Flutter应用的Dart代码需要与原生代码交互时，会通过`Platform Channels`发送一个消息。

  - 消息会通过在`flutter`库中编码（一般编码为二进制形式）。

  - 经过编码的消息会通过`C++`层的引擎被转发到对应平台的接收端（Android、iOS）。

  - 在Android或iOS端会解码这个消息，执行相应的原生代码操作，并可能会将结果再次编码发送回Flutter。

  - Flutter接收到来自原生端的消息后进行解码，得到结果。

  这样通过消息在Flutter和原生平台之间来回传递，就能实现Flutter与原生的交互。这种机制可以让两边都各自维护自己的代码和内存，避免了因为语言不同导致的内存管理问题，同时也提供了异步操作的可能，以适应某些需要耗时的操作（如网络请求）。

  注意：`Platform Channels` 主要有三种类型，分别是数据通道（`Method Channel`）、事件通道（`Event Channel`）和基础消息通道（B`asic Message Channel`），它们在通信模式和适用场景上各有不同。
...
> 在 `Android` 和 `iOS` 中，`Flutter` 与原生的消息传递都不涉及到进程间通信 (`IPC`)。无论是 `Flutter` 还是原生的代码，都运行在相同的进程中。其实，`Flutter` 的消息传递更像是一个跨语言的函数调用。基于这种机制，消息在 `Flutter` 的运行时与原生宿主之间来回传递。

  - 在 `Android` 中，`Flutter` 的运行时与原生宿主通信是在主线程中进行的，涉及的实质上是线程间通信。为了不阻塞界面操作，开发者需要手动开辟新的线程来执行耗时操作。

  - 在 `iOS` 中，情形大致相同，外加一种情况。在 `Flutter` 应用的启动过程中，与 `UIApplication` 的通信是在主线程中进行的。然而，应用启动完成后，与原生宿主的通信就在一个独立的线程中进行，也就是说，此时则涉及到了线程间通信。

  注意：虽说消息传递是在不同的线程间进行的，但由于 `Flutter` 的线程模型设计，`Flutter` 中的每一种线程，包括主线程(`UI` 线程)、`GPU` 线程、`I/O` 线程、平台线程等，都通过事件队列(`Event Queue`)与原生宿主进行通信，形成了 `Flutter` 的线程间通信机制。这可以确保与原生宿主的通信过程是有序且安全的。
