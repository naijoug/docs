...
【思路】深度优先遍历，进行不断递归判断左右子节点中的值是否相等。
...
【思路2】反转+对比，将二叉树右子树进行反转，然后与左子树进行对比。
...
【思路1】通过哈希表进行统计。如果统计的某个数计数大于一半时，就是结果数

【思路2】摩尔投票，定义两个变量，一个计数，一个结果值。
  - 遍历数据，当计数为 0 时，设置结果值，并计数 +1
  - 当计数不为 0 时，对比遍历值结果值，如果相等则计数 +1；如果不相等 -1。
  - 最后结果值就是大于一半的数。
