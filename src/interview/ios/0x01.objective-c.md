...
> `OC` 的 `runtime` 文档中介绍中就详细写明了，`runtime` 的设计是为了将更多的处理延迟到编译后。
  ...
  - 动态绑定 (`Dynamic binding`) : 方法的调用也是运行时才决定，而并非编译时。
    ...
    * `OC` 的消息机制 : 运行时，才去动态查找与实例绑定的属性和方法
  
  - 动态加载 (`Dynamic loading`) : 运行时可以动态加载其它模块或框架。
    * 运行时启动时才进行加载(如 : `+load` 函数中的一些初始化)
...
::: details 💡
  ...
  注：以上这些都只能尽可能地保证 `ABI` 的稳定性，但并不能绝对地保证。因为系统升级往往会引入新的功能、新的架构，或者修复旧的 `bug`，这些都可能对 `ABI` 产生影响。所以，作为开发者在编写代码时，也需要注意兼容性，避免过度依赖可能会变动的接口和行为。
...
> `NSNumber`是 `Foundation` 框架中的一个类，它是包装类型，也是对基本的 `C` 和 `Objective-C` 数据类型(`Integer`、`Float`、`Bool`等)进行封装的一个类。在 `Objective-C` 不能直接在`NSArray`、`NSDictionary` 等集合类中存放非对象类型的数据，这时我们就需要用到 `NSNumber` 这个类。

大致实现原理：`NSNumber`的对象很可能内部包含一个 `union` 类型的实例变量，这个 `union` 包含了所有可以表示的数据类型（整数，浮点，布尔，etc）。

```objc
// 构造方法可能是根据传入的参数类型来设置对应的 union 成员变量的值
...
// 取值方法也是类似，根据取值的类型返回对应的 union 成员变量的值
- (int)intValue {
...
`NSDictionary` 和 `NSCache` 都是能够存储键值对的集合，但它们有一些主要的区别，使它们适用于不同的使用场景：
  ...
  - `NSDictionary` 是一个不可变的哈希表，其中的键值对都是强引用的。键是唯一的且不可变的。相比之下，`NSCache` 是一个可变的集合，其中的键值对都是强引用的，但有一个例外：键不是像 `NSDictionary` 管理那样通过 `copy` 管理，而是通过 `retain` 管理，因此键可以是可变的。

  - 另一个重要的区别在于内存管理。`NSCache` 在系统内存不足时，会自动释放缓存的部分或全部数据以节省内存。这使得在处理大量数据时，`NSCache` 可以自动处理低内存警告，而 `NSDictionary` 则需要手动处理低内存警告。

  - `NSCache` 支持通过 `NSDiscardableContent` 协议来控制缓存内容的生命周期。这样的对象在被缓存时，可以增加一个 `beginContentAccess` 之后配对的 `endContentAccess` 调用计数器。当对象的计数器为 0 时，那么这个对象就有可能被清出缓存。

总结：`NSDictionary` 和 `NSCache` 的不同主要在于内存管理和可变性。实际上，`NSCache` 更类似一个优化了内存管理的 `NSMutableDictionary`，更适合用于存储大量会占据大量内存的对象，例如图片等。
...
::: details 💡
  ...
  - `Dictionary` 和 `Set` 这两个集合类在进行元素存储时，需要通过 `hash` 函数来确定元素的存储位置。
  - 对象间进行比较时，常常会用到 `hash` 属性。
  - 在进行一些数据加密或者验证时，比如 `MD5`、`SHA-1` 等也是利用了哈希算法来实现的。
  - 文件校验也经常用哈希。
...
::: details 💡
  ...
  - **传统的 for 循环**：对于可以通过索引访问元素的集合（如数组）。
  ...
  - **Block 枚举**：很多集合类都提供了基于 `block` 的遍历方法，比如`enumerateObjectsUsingBlock:`。
  ...
  - **NSEnumerator**：一个用于遍历集合的类，使用 `nextObject` 方法可以取出下一个元素。
...
### ❓`for in` 遍历的时候在里面增加一个元素会怎么样，删除一个元素会怎么样，为什么

::: details 💡

> 在使用 `for in` 遍历集合类（比如数组或字典）的时候，`Objective-C` 是不允许在遍历过程中更改集合（即在增加元素或删除元素）的。如果试图在 `for in` 遍历过程中添加或删除元素，会在运行时抛出异常，具体表现为 `NSException`。

原因：对于容器的遍历通常使用的是迭代器模式，迭代的过程需要一个稳定的环境。如果在遍历过程中修改了容器（增加或者删除元素），就可能导致迭代器的状态异常，比如迭代顺序出现错误，或者遍历到不存在的元素。

```objc
// 可以先将需要删除的元素记录下来，等遍历结束后再进行删除
...
#### ❓两个类的 `load` 方法里互相创建另一个类会怎么样

::: details 💡

> `load` 方法是在一个类被引入到运行时系统时调用，每个类的 `load` 方法只被调用一次。它们往往在 `main` 函数启动之前就开始执行。

如果两个类的`load`方法里互相创建对方：

  - 在加载类 A 时，如果`load`方法里创建了类 B 的实例，会导致立即加载类 B。

  - 在加载类 B 时，如果`load`方法里创建了类 A 的实例，但此时因为类 A 已经在加载中，所以并不会触发类 A 的`load`方法。

注意：此时会产生一个循环引用的情况，也有可能导致其他不可预知的副作用或错误，因此在编程过程中应避免这样做。

  在实际开发中，`load` 方法通常用来执行只需要执行一次并且涉及到类本身的操作，例如注册类到一个注册表，或者设置为观察者等等。在 `load` 方法中执行耗时操作、修改环境状态或者产生副作用通常是不推荐的。如果需要在类加载时执行初始化代码，可以考虑使用 `+initialize` 方法，它在类第一次收到消息时被调用，而且如果子类没有定义 `initialize` 方法，那么父类的 `initialize` 方法会被调用多次。
...
::: details 💡
  ...
  - `alloc`：`alloc` 意为 `allocation`，用于在堆内存中分配内存，创建对象实例。这个方法会返回一个内存块的地址，这个内存块刚好够存放对象的所有实例变量。但是，这个内存块中的内容是未初始化的。

  - `init`：`init` 是用来初始化对象的。使用 `alloc` 分配地址创建的对象，所有的实例变量都会被初始化为 0，但通常需要对这些实例变量进行初始化，所以就调用 `init` 方法。`init` 方法会返回一个初始化后的对象。

  - `new`：`new` 是 `alloc` 和 `init` 的简写，它等同于 `alloc` 方法后面跟一个 `init` 方法。使用 `new`方法可以一步完成对象的分配和初始化。

> `alloc-init` vs `new`
    
  - 使用 `new` 执行的是 `alloc` 和 `init`，并且两者都不能带参数。
  - 使用 `alloc` 和 `init`，可以对 `init` 方法传递参数，比如 `initWithFrame:`， `initWithNibName:bundle:` 等等。 这也是 `new` 和 `alloc-init` 的主要区别，也是为什么在开发中往往使用 `alloc-init` 来创建对象，而不是 `new`。
    
:::

#### ❓`NS_DESIGNATED_INITIALIZER` 如何添加
...
::: details 💡
  ...
  - 反射：
    ...
    * 运行时（`Runtime`）：`Objective-C` 的运行时系统使得能够在运行时检查和修改类和对象的信息。因此，可以在运行时查询和修改类和实例变量，动态创建类，动态替换方法等等。
  
    * `NSClassFromString` 和 `NSSelectorFromString` 这两个函数可以从字符串中得到类和方法。
  ...
  - 使用场景：
    ...
    * 解析复杂的 `JSON` 或 `XML` 结构的时候，比如想把 `JSON` 转换为特定的对象，而 `JSON` 的键值恰好对应了这个对象的属性，可以使用反射遍历对象的属性，并填充对应的值。
    * 如果需要编写更通用性的代码，例如正在编写一个通用的数据源，它需要处理任何类型的数据，这种情况下需要使用反射。

  - 替代方案：如果明确知道要操作的类型，那么通常不需要使用反射。可以直接通过属性名称访问属性，或者直接调用方法。

  总结：虽然反射是一个强大的工具，能够提供极大的灵活性，但是它应该谨慎使用。不正确的使用反射可能会导致代码难以理解和维护，甚至产生错误。只有在必要的时候才应该使用反射。
...
> `atomic` 和 `nonatomic` 是 `Objective-C` 中声明属性时使用的两种属性原子性（`atomicity`）修饰符。
 
  - `atomic`：代表属性是原子操作。这意味着对该属性的 `setter` 和 `getter` 方法在多线程环境下保证了线程安全，不会出现数据不一致的现象。但，这并不代表所有的操作都是线程安全的，比如对属性的读-写-写操作就不是线程安全的，同时atomic会带来一定的性能开销。如果没有指定atomic或nonatomic，则默认为atomic。

  - `nonatomic`：代表属性非原子操作。在多线程环境下对该属性的存取操作可能会出现数据不一致情况，但在性能上有所优势，因为nonatomic避免了加锁开销。

为了提高效率，在没有并发访问的情况下，我们通常使用`nonatomic`。对于有并发访问的属性，我们需要在getter和setter中实现适当的锁来管理对这些属性的访问，而不是仅仅依赖于`atomic`，因为`atomic`并不能完全解决并发的问题。
...
不论一个属性是通过`atomic`还是`nonatomic`修饰，它都可以被子类重载。重载一个属性主要就是提供一个新的 `setter` 或者 `getter`。在子类中重载一个 `atomic` 属性时，除非明确重载了这个属性的全部存取器（包括 `getter` 和 `setter`），否则它仍然是一个原子操作。
...
注：`@dynamic` 常常和运行时函数 `class_addMethod` 配合使用，可以动态添加方法。最常见的应用是在 `Core Data` 的 `NSManagedObject` 子类中，`Core Data` 的属性经常被设置为 `@dynamic`，`NSManagedObject` 利用 `Core Data` 的底层存储访问这些属性，以便将对象的实际数据保存在磁盘上，而不是内存中。 
...
注：在 `Xcode 4.4` 及以上版本中，默认已经开启了 `@synthesize` 自动生成，已经不需要显式地使用 `@synthesize`，除非在某些特殊情况下需要手动指定实例变量的名字。
...
注：浅拷贝并不意味着拷贝出来的对象一定是不可变的，具体还需要看拷贝的源对象是什么类型。如果拷贝的源对象是不可变的，那么 `copy` 方法拷贝出来的对象也是不可变的；如果拷贝的源对象是可变的，那么 `copy` 方法拷贝出来的对象就是可变的。
...
### ❓`[object copy]` 是浅拷贝还是深拷贝

::: details 💡

> `[object copy]`方法的行为取决于对象支持哪种拷贝。

  - 默认情况下，`[object copy]`执行的是浅拷贝，也叫做指针拷贝，即创建一个新对象，新对象的内容指向原对象内存。这种情况下，修改新复制出来的对象会影响到原对象。

  - 如果要实现深拷贝，需要对象遵守 `NSCopying` 或 `NSMutableCopying` 协议，并实现 `copyWithZone:` 或 `mutableCopyWithZone:` 方法，在这两个方法中执行实际的复制操作。在这种情况下，`[object copy]` 执行的就是深拷贝，即完全复制出一个新对象，新对象的内容和原对象是独立的。修改新复制出来的对象不会影响到原对象。

🌰 `NSArray` 和 `NSString` 等不可变对象的 `copy` 方法执行的是浅拷贝，而 `mutableCopy` 方法执行的是深拷贝。而对于它们各自的可变版本，即 `NSMutableArray` 和 `NSMutableString` 等， `copy` 和 `mutableCopy` 方法都执行的是深拷贝。

总结：`[object copy]` 是浅拷贝还是深拷贝，需要根据该对象的具体实现来判断。
...
⚠️ 如果希望在 `block` 中修改 `array` 这个变量（比如使其指向一个全新的 `NSMutableArray` 对象），那就需要使用 `__block` 了。
...
::: details 💡
  ...
  `block` 初始创建时是在栈上。但是，当调用 `block` 的 `copy` 方法或者把 `block` 传递给需要异步执行的 `GCD` 方法时，`block` 会被复制到堆上。

  `GCD` 是 `Apple` 提供的一种多核编程的解决方案，它允许将任务封装到 `block` 里面，然后放入 `Dispatch Queue`（调度队列）中进行异步执行。为了避免 `block` 在执行时由于变量被释放而产生错误，`GCD` 会手动对 `block` 进行复制操作，把 `block` 从栈复制到堆上，这样即便 `block` 的创建环境已经不存在，`block` 中也仍然能够访问到创建环境的变量值。
...
`Extension` 可以用来添加新的方法到现有的类中，但它没有自己的实例变量，所以不能存储新的状态。在使用 `Extension` 时，需要注意以下几点：
  ...
  - 如果覆盖已有的方法，所有继承自 `NSObject` 的对象都会受到影响，可能出现无法预知的结果。
...
::: details 💡
  ...
  > 分类（`Category`）主要用于给现有的类添加方法，但是它不能直接添加属性。如果你想在分类中添加一个 `weak` 属性，需要使用的关联对象（`Associated Objects`）来实现。
...
- (void)setWeakProperty:(id)weakProperty {
    // 使用 OBJC_ASSOCIATION_ASSIGN 它表示一个弱引用，但不像 weak 那样会自动置 nil，需要在适当的时候置 nil
...
注：这种方式并不是真正意义上在分类中添加属性，只是把一个对象关联到了分类的实例对象上，并没有改变类的结构。在内存分配上，主类并不知道新增的这个关联对象。这种方案产生的影响仅限于对象在生命周期内，一旦对象销毁，关联的对象也就随之销毁。
...
> 计算一个方法调用的时间，通常的方案是在调用方法之前和之后各取一次系统时间，并计算时间差。不过在汇编语言中，这部分逻辑将变得更加复杂，主要源于汇编语言的低阶性质，以及 `Objective-C` 方法调用的动态性。

  若要在函数入口和返回处插入计时的代码，理论上需要修改编译出来的二进制文件，这在实际操作中较为困难。

  一种更为可行的方法是 `hook` 函数的调用，在 `Objective-C` 中，对象方法调用最后实际上会通过 `objc_msgSend` 函数的机制实现，可以创建自定义版本的 `objc_msgSend`，并在这个函数中计算方法的执行时间。

大致步骤：

  - 在程序启动时，获取原始的 `objc_msgSend` 的函数指针，并把它保存下来。

  - 编写一个新的函数，替代 `objc_msgSend`。在这个新函数中：
    * 使用当前时间减去开始时间，计算出时间差，也就是方法的执行时间。
    * 执行原本的 `objc_msgSend` 函数。

  - 使用 `Objective-C` 的方法交换机制，将新的函数与 `objc_msgSend` 函数进行交换。
  
注意：因为涉及到汇编语言，必须具备相应的 `ARM` 汇编知识，而且要考虑到指令集的不同，针对 32 位和 64 位可能需要分别处理。
