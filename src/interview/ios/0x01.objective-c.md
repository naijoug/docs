...
- `nil`：在Objective-C中，nil是一个表示对象指针为空的特殊值。在Swift中，nil是一个表示任何类型的可选值为空的特殊值。对nil发送消息在Objective-C中是安全的，在Swift中是不允许的。
...
- 内存管理：指定的属性修饰符（`assign`、`strong`、`weak`、`copy`、`retain` 等），编译器会在 `setter` 方法中生成相应的内存管理代码，比如引用计数的增加和减小，拷贝对象等。
